...
// 查管理室座標
$gps = DB::table('customers')
    ->select('Lat', 'Lng')
    ->where('customer_id', $cusId)
    ->first();

$officeLat = floatval($gps->Lat);
$officeLng = floatval($gps->Lng);
...
// 先標記 abnormal 與找出 abnormal='x' 的 QrcodeID
$abnormalXIds = [];

// 第一步：先標記 abnormal (x/*/空字串)
foreach ($json_arr['Qrcode'] as &$item) {
    ...
    if (!isset($validQrcodes[$qrcodeId])) {
        // 不在白名單
        $item['abnormal'] = 'x';
        $abnormalXIds[] = $qrcodeId;
    } else {
        // 在白名單，判斷距離
        if (is_null($lat) || is_null($lng)) {
            $item['abnormal'] = '*'; // GPS缺失視為異常
        } else {
            $distanceFromOffice = haversineDistance($lat, $lng, $officeLat, $officeLng);
            if ($distanceFromOffice > 300) {
                $item['abnormal'] = '*'; // 超過範圍異常
            } else {
                $item['abnormal'] = ''; // 正常
            }
        }
...
// 第二步：檢查 GPS 重複（沒移動），全部改為 '*'
$gpsCountMap = [];
foreach ($json_arr['Qrcode'] as $item) {
    if (!isset($item['Lat']) || !isset($item['Lng'])) continue;
    $key = floatval($item['Lat']) . '_' . floatval($item['Lng']);
    if (!isset($gpsCountMap[$key])) {
        $gpsCountMap[$key] = [];
    }
    $gpsCountMap[$key][] = $item;
}

// 找出重複 GPS，標記
foreach ($gpsCountMap as $key => $items) {
    if (count($items) > 1) {
        foreach ($json_arr['Qrcode'] as &$item) {
            $itemKey = floatval($item['Lat']) . '_' . floatval($item['Lng']);
            if ($itemKey === $key && $item['abnormal'] !== 'x') {
                $item['abnormal'] = '*';
            }
        }
        unset($item);
...
// 寫入 DB，只寫 abnormal 非 'x' 的
foreach ($json_arr['Qrcode'] as $data) {
    if ($data['abnormal'] === 'x') {
        continue;
    }

    $search_cusid = DB::table('qrcode')
        ->select('customer_id', 'patrol_RD_Name')
        ->where('patrol_RD_No', '=', $data['QrcodeID'])
        ->first();
...
return response()->json([
    ...
    {   
        ...
        dd($search_cusid);
        if(isset($search_cusid->customer_id))
        {
                return response()->json([
